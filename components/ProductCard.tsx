    ...
    <>
      ...
      >
        ...
        <div className="img-zoom-container aspect-square relative bg-slate-50 shrink-0 overflow-hidden">
            ...
            {!imgError && product.imageUrl ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-slate-100">
                <div className="text-center p-3">
                  ...
                  <p className="text-slate-500 text-2xs mt-1 truncate">{product.categoryName}</p>
                </div>
              </div>
            )}
          </Link>
          ...
          <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/15 transition-colors duration-200 flex items-center justify-center pointer-events-none">
            <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <Link
                href={`/urun/${product.id}`}
                className="pointer-events-auto bg-white/95 text-slate-900 border border-slate-200 text-2xs font-bold px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1 hover:text-gold transition-colors"
              >
                <Eye size={12} /> İncele
              </Link>
            </div>
          ...
          {!imgError && product.imageUrl && (
            <button
              ...
              className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 p-1.5 bg-white/90 hover:bg-gold text-slate-700 hover:text-white border border-slate-200 rounded-lg shadow-sm"
              title="Görseli büyüt"
            >
              <ZoomIn size={13} />
            </button>
        ...
        <div className="p-3 sm:p-3.5 flex flex-col flex-1 justify-between bg-white">
          <div>
            ...
            <div className="flex items-center gap-1 mb-1">
              ...
              <span className="text-[10px] text-gold-600 font-bold uppercase tracking-wider truncate">
            ...
            <Link href={`/urun/${product.id}`}>
              <h3 className="font-heading font-semibold text-slate-800 text-xs sm:text-sm leading-snug line-clamp-2 hover:text-gold transition-colors mb-1">
                {product.name}
              </h3>
            </Link>
